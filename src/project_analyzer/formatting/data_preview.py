...
def preview_data_file(file_path: str) -> str:
    ...
    if not os.path.isfile(file_path): return f"## Error: File not found: `{file_path}`"

    ext = os.path.splitext(file_path)[1].lower()
    header = f"## Preview: {os.path.basename(file_path)}\n"

    try:
        if ext == ".csv": return header + (_preview_csv_pandas(file_path) if PANDAS_AVAILABLE else _preview_csv_basic(file_path))
        if ext == ".tsv": return header + (_preview_csv_pandas(file_path, "\t") if PANDAS_AVAILABLE else _preview_csv_basic(file_path, "\t"))
        if ext == ".parquet": return header + (_preview_parquet(file_path) if PYARROW_AVAILABLE else "Preview requires `pyarrow`.")
        if ext == ".json": return header + _preview_json(file_path)
...
def _preview_json(file_path: str) -> str:
    ...
    try:
        ...
        if isinstance(data, dict): return f"- **Type:** JSON Object\n- **Top-level keys:** `{', '.join(list(data.keys())[:5])}`"
        if isinstance(data, list): return f"- **Type:** JSON Array\n- **Items:** {len(data)}"
