...
DEFAULT_IGNORE_PATTERNS = {
    "*.pyc", "*.pyo", "*.pyd", "*.so", "*.egg-info", ".DS_Store", "*.swp", "*.bak",
}
...
def get_module_name(relative_path: str) -> Optional[str]:
    ...
    if not relative_path.endswith(".py"): return None
...
def is_path_ignored(relative_path: str, ignore_patterns: Set[str]) -> bool:
    ...
    if any(part in ALWAYS_IGNORE_DIRS for part in path_parts): return True

    basename = os.path.basename(relative_path)
    if any(fnmatch.fnmatch(basename, pattern) for pattern in DEFAULT_IGNORE_PATTERNS): return True
